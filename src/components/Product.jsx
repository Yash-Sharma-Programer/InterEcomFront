...
const Product = () => {
    ...
    useEffect(() => {
        ...
        const url = searchQuery
            ? `https://ecom-backend-ovxs.vercel.app/products?search=${encodeURIComponent(searchQuery)}`
            : 'https://ecom-backend-ovxs.vercel.app/products'
        fetch(url)
            .then(res => res.json())
            .then(data => {
                ...
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h2 className="text-2xl sm:text-3xl font-bold text-gray-800">
                            {searchQuery ? <>Search Results <span className="text-indigo-500">"{searchQuery}"</span></> : 'All Products'}
                ...
                {products.length === 0 && !searchQuery ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                        {products.map(product => (
                            <div key={product._id} className="group bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-200">
                                ...
                                <div className="p-4">
                                    ...
                                    <button onClick={() => setBuyNowProduct(product)}
                                    ...
                                    <button
                                        ...
                                        className="mt-2 w-full py-1.5 rounded-xl text-xs font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-50 border border-gray-100 transition"
                                    >
                                        {expandedReview === product._id ? '▲ Hide Reviews' : '★ View Reviews'}
                                    </button>
                                </div>
